...
app.post('/verify-payment', async (req, res) => {
  try {
    ...
    if (data.status === 'successful' && data.currency === 'NGN') {
      // CRITICAL SECURITY CHECK: derive the real expected price from Smeplug's
      // live plan list, never trust the client-supplied order.amount alone.
      let expectedAmount = order.amount; // fallback if we can't verify
      if (order.type === 'data' && order.networkId && order.planId) {
        try {
          const plansRes = await axios.get('https://smeplug.ng/api/v1/data/plans', {
            headers: { 'Authorization': `Bearer ${SMEPLUG_KEY}` }
          });
          const networkPlans = plansRes.data.data?.[order.networkId] || [];
          const matchedPlan = networkPlans.find(p => String(p.id) === String(order.planId));
          if (matchedPlan) expectedAmount = parseFloat(matchedPlan.price);
        } catch (e) {
          console.error('verify-payment: could not fetch plan price', e.message);
        }
      }

      if (data.amount < expectedAmount) {
        return res.json({ success: false, message: `Underpaid: expected ₦${expectedAmount}, got ₦${data.amount}` });
      }
...
async function sendData(order) {
  try {
    ...
    console.log('Smeplug data purchase response:', JSON.stringify(response.data));

    if (response.data.status === true || response.data.status === 'success') {
      return { success: true, message: 'Data delivered!', ref: response.data.ident || response.data.reference };
    } else {
      return { success: false, message: response.data.message || response.data.msg || JSON.stringify(response.data) };
    }
  } catch (err) {
    const fullError = err.response?.data ? JSON.stringify(err.response.data) : err.message;
    console.error('Smeplug data purchase FULL error:', fullError);
    return { success: false, message: 'Smeplug error: ' + fullError };
  }
}
...
// ── TEST FLW_SECRET VALIDITY ──
app.get('/check-flw-key', async (req, res) => {
  try {
    const response = await axios.get(
      'https://api.flutterwave.com/v3/transactions?status=successful',
      { headers: { Authorization: `Bearer ${FLW_SECRET}` } }
    );
    res.json({ success: true, message: 'FLW_SECRET is VALID and working', sample: response.data.data?.[0]?.id || 'no transactions yet' });
  } catch (err) {
    res.json({ success: false, message: 'FLW_SECRET is INVALID or expired', error: err.response?.data?.message || err.message, status: err.response?.status });
  }
});
...
app.post('/webhook', async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('Webhook: verify API call failed -', verifyErr.response?.status, verifyErr.response?.data?.message || verifyErr.message);
      // Fall back to trusting the webhook payload itself since signature was already validated
      if (payload.data.status === 'successful') {
        data = payload.data;
        console.log('Webhook: using webhook payload data as fallback (signature was valid)');
      } else {
        return;
      }
    }
    ...
    if (data.status !== 'successful' || data.currency !== 'NGN') {
      console.log('Webhook: payment not successful for', transaction_id);
      return;
    }

    // Reconstruct order details from meta_data (top-level field, sent during checkout)
    const customer = payload.data.customer || data.customer || {};
    const meta = payload.meta_data || data.meta || {};
    const paidAmount = parseFloat(data.amount || payload.data.amount || 0);

    // CRITICAL SECURITY CHECK: verify the paid amount matches what the plan should cost.
    // Without this, someone could tamper with meta_data to request an expensive plan
    // while paying a smaller amount. We re-derive the expected price from Smeplug's
    // live plan list rather than trusting any client-supplied amount field.
    let expectedAmount = null;
    if (meta.type === 'data' && meta.networkId && meta.planId) {
      try {
        const plansRes = await axios.get('https://smeplug.ng/api/v1/data/plans', {
          headers: { 'Authorization': `Bearer ${SMEPLUG_KEY}` }
        });
        const networkPlans = plansRes.data.data?.[meta.networkId] || [];
        const matchedPlan = networkPlans.find(p => String(p.id) === String(meta.planId));
        if (matchedPlan) {
          expectedAmount = parseFloat(matchedPlan.price);
        }
      } catch (e) {
        console.error('Webhook: could not fetch plan price for verification', e.message);
      }
    }

    if (expectedAmount !== null && paidAmount < expectedAmount) {
      console.log(`Webhook: REJECTED - paid ₦${paidAmount} but plan costs ₦${expectedAmount} (tx: ${transaction_id})`);
      saveOrder({
        type: meta.type, networkId: meta.networkId, planId: meta.planId,
        transaction_id: String(transaction_id), tx_ref,
        status: 'rejected_underpaid', paidAmount, expectedAmount,
        timestamp: new Date().toISOString()
      });
      return;
    }
